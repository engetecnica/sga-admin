                ...
                <table class="table-hover table-striped table" id="lista-simples">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span class="badge badge-danger">{{ $v->obra->codigo_obra }}</span></td>
                            <td>{{ $v->matricula ?? '-' }}</td>
                            <td>{{ $v->nome }}</td>
                            <td>{{ $v->celular }}</td>
                            <td>{{ $v->email }}</td>
                            ...
                            <td class="d-flex justify-itens-between">
                                ...
                                @if (session()->get('usuario_vinculo')->id_nivel <= 2) <form action="{{ route('cadastro.funcionario.destroy', $v->id) }}" method="POST">
                                    @csrf
                                    @method('delete')
